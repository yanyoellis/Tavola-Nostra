  ...
  return <section className={`table-scene ${active ? `table-scene--active table-scene--${active}` : ''}`}>
    ...
    <div className="hero-copy">
      ...
      <div className="hero-copy__actions">
        <Link to="/case-study" className="button button--ghost">View UX Case Study <span>→</span></Link>
      </div>
    ...
    <div className="desktop-hotspots">
      ...
      <Link to="/menu" className="hotspot hotspot--menu" ariaLabel="Open full menu">
        <span className="hotspot__pulse"/>
        <span className="hotspot__label"><small>Leather-bound</small>Full Menu<b>↗</b></span>
      </Link>
      <Link to="/cart" className="hotspot hotspot--receipt" ariaLabel="Open your order">
        <span className="hotspot__pulse"/>
        <span className="hotspot__label"><small>Your order</small>The Bill<b>↗</b></span>
      </Link>
    </div>
    <div className="mobile-scene">
      <div className="mobile-scene__intro">
        <span className="kicker kicker--cream">A seat at our table</span>
        <h1>Choose your <em>course.</em></h1>
      </div>
      <div className="mobile-scene__list">
        {categories.slice(1, 5).map((category) => <button
          key={category.id}
          onClick={() => setActive(category.id)}
          style={{ backgroundImage: `linear-gradient(90deg,rgba(24,12,8,.12),rgba(24,12,8,.72)),url(${category.image})` }}
        >
      ...
      <div className="mobile-scene__actions">
    ...
    <aside className={active ? 'scene-drawer scene-drawer--open' : 'scene-drawer'} aria-hidden={!active}>
      ...
      {current && <>
        <span className="kicker">{current.eyebrow}</span>
        <h2>{current.label}</h2>
